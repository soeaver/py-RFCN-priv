...
bool FrcnnParam::rpn_normalize_targets;
float FrcnnParam::rpn_normalize_means[4];
float FrcnnParam::rpn_normalize_stds[4];

bool FrcnnParam::agnostic;
...
void FrcnnParam::load_param(const std::string default_config_path) {
  ...
  FrcnnParam::rpn_normalize_targets =
      static_cast<bool>(extract_int("rpn_normalize_targets", default_map));
  v_tmp = extract_vector("rpn_normalize_means", default_map);
  std::copy(v_tmp.begin(), v_tmp.end(), FrcnnParam::rpn_normalize_means);
  v_tmp = extract_vector("rpn_normalize_stds", default_map);
  std::copy(v_tmp.begin(), v_tmp.end(), FrcnnParam::rpn_normalize_stds);

  FrcnnParam::agnostic =
      static_cast<bool>(extract_int("agnostic", default_map));
...
void FrcnnParam::print_param(){
  ...
  LOG(INFO) << "agnostic             : " << FrcnnParam::agnostic;
}
